...
round_percent <- function(x) {
  p <- x / sum(x) * 100
  p_floor <- floor(p)
  remainder <- round(100 - sum(p_floor))
  
  # add 1 to the largest fractional parts
  if (remainder > 0) {
    idx <- order(p - p_floor, decreasing = TRUE)[1:remainder]
    p_floor[idx] <- p_floor[idx] + 1
  }
  return(p_floor)
}

# function to generate sum table
make_wide_summary <- function(data = cleaned_data, row_var, col_var = "disease_domain") {
  ...
  for (col in domain_cols) {
    pretty_df[[col]] <- paste0(round_percent(wide_df[[col]] / col_totals[col]),"%")
  }
